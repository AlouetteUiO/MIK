...
for filename in os.listdir(directory):
    if filename.endswith(suffix):
        ...
        if (df['NOTE.typeOfFlight'][0] == 'octagon') or (df['NOTE.typeOfFlight'][0] == 'plough'):
            ...
            if not os.path.exists(f"{directory}/04_bayes_obs_b/{file_string}"):
                os.makedirs(f"{directory}/04_bayes_obs_b/{file_string}")
            ...
            fig.suptitle(f"abs_sigma = {abs_sigma[0][0], np.round(R_windspeed,2)} | Neff = {np.round(Neff, 1)}/{n_particles} | \
                flux = {mean_flux} +- {std_flux} mg/s | V = {mean_V} +- {std_V} | WD = {mean_WD} +- {std_WD} | D = {mean_D} +- {std_D}")
            plt.tight_layout()
            plt.savefig(f"{directory}/04_bayes_obs_b/{file_string}/case_b.png")
            plt.close()
            print("saved figure")

# save results dictionary
np.save(f"{directory}/04_bayes_obs_b/results_dict.npy", results_dict)