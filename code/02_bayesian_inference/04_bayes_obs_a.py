...
n_source = 100   # 100 used in results article
...
for filename in os.listdir(directory):
    if filename.endswith(suffix):
        ...
        if (df['NOTE.typeOfFlight'][0] == 'octagon') or (df['NOTE.typeOfFlight'][0] == 'plough'):
            ...
            if not os.path.exists(f"{directory}/04_bayes_obs_a/{file_string}"):
                os.makedirs(f"{directory}/04_bayes_obs_a/{file_string}")
            ...
            # make prior
            n_vars = 4
            n_particles = 25_000  # 25_000 used in results article
            flux_min = 0.1
            flux_max = 10
            ...
            prior[2,:] = rng.uniform(low=winddirection_min, high=winddirection_max, size=n_particles)  # wind direction [deg], adjust this to 30, 135 for the plough/V-shaped flights
            ...
            n_mcmc_steps = 5  # 5 used in results article
            batch_size = 200  # 200 used in results article
            ...
            fig.suptitle(f"abs_sigma = {abs_sigma[0][0]} | Neff = {np.round(Neff, 1)}/{n_particles} | \
                flux = {mean_flux} +- {std_flux} mg/s | V = {mean_V} +- {std_V} | WD = {mean_WD} +- {std_WD} | D = {mean_D} +- {std_D}")
            plt.tight_layout()
            plt.savefig(f"{directory}/04_bayes_obs_a/{file_string}/case_a.png")
            plt.close()
            print("saved figure")

# save results dictionary
np.save(f"{directory}/04_bayes_obs_a/results_dict.npy", results_dict)